...
You can perform training on a small dataset, e.g., a collection of string quartets, or even a single performance, thus obtaining labels for the performance/s. The labelled performance/s can be added into a larger dataset. We recommend using the alignment for labels and not the predictions, since 
